...
def connect_db():
    ...
    # 连接所用集合，也就是我们通常所说的表
    collection = db.internet_drug_store
    return collection
...
def detail_data(id):
    url = 'http://app1.sfda.gov.cn/datasearchcnda/face3/content.jsp?' \
          'tableId=96&tableName=TABLE96&tableView=网上药店&Id=' + str(id)
    try:
        driver.get(url)
        ...
        if '没有相关信息' in driver.page_source:
            ...
            coll.insert({
                ...
                # 服务范围
                'service_range': trs[2].find_all('td')[1].text.strip(),
                # 公司名称
                'company_name': trs[3].find_all('td')[1].text.strip(),
                # 法定代表
                'legal_person': trs[4].find_all('td')[1].text.strip(),
                # 单位地址
                'address': trs[5].find_all('td')[1].text.strip(),
                # 省份
                'province': trs[6].find_all('td')[1].text.strip(),
                # 网站名称
                'website_name': trs[7].find_all('td')[1].text.strip(),
                # ip地址
                'IP_address': trs[8].find_all('td')[1].text.strip(),
                # 域名
                'domain': trs[9].find_all('td')[1].text.strip(),
                # 发证日期
                'send_cerific_date': trs[10].find_all('td')[1].text.strip(),
                # 有效截止日期
                'effective_end_date': trs[11].find_all('td')[1].text.strip(),
                # 邮编
                'postal_code': trs[12].find_all('td')[1].text.strip(),
                # # 注
                # 'notes': trs[13].find_all('td')[1].text.strip(),
