...
def connect_db():
    ...
    # 连接所用集合，也就是我们通常所说的表
    collection = db.drugs_yi_liao_qixie_operate_record
    return collection
    ...
    try:
        ...
        if '没有相关信息' in driver.page_source:
            ...
            coll.insert({
                ...
                'detail_id': id,
                # 编号
                'record_num': trs[1].find_all('td')[1].text.strip(),
                # 企业名称
                ...
                # 发证部门
                'record_dept': trs[11].find_all('td')[1].text.strip(),
                # 发证日期
                'record_date': trs[12].find_all('td')[1].text.strip(),
                # 注
