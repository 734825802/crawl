...
def connect_db():
    ...
    # 连接所用集合，也就是我们通常所说的表
    collection = db.licensed_pharmacist_registrar
    return collection
...
def detail_data(id):
    url = 'http://app1.sfda.gov.cn/datasearchcnda/face3/content.jsp?' \
          'tableId=122&tableName=TABLE122&tableView=执业药师注册人员&Id=' + str(id)
    try:
        driver.get(url)
        ...
        if '没有相关信息' in driver.page_source:
            ...
            coll.insert({
                ...
                'detail_id': id,
                # 姓名
                'name': trs[1].find_all('td')[1].text.strip(),
                # 注册证书编号
                'reg_license_num': trs[2].find_all('td')[1].text.strip(),
                # 执业地区
                'practicing_area': trs[3].find_all('td')[1].text.strip(),
                # 执业类别
                'practicing_category': trs[4].find_all('td')[1].text.strip(),
                # 执业范围
                'practicing_range': trs[5].find_all('td')[1].text.strip(),
                # 执业单位
                'company': trs[6].find_all('td')[1].text.strip(),
                # 有效日期
