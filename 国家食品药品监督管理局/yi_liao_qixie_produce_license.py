...
def connect_db():
    ...
    # 连接所用集合，也就是我们通常所说的表
    collection = db.drugs_yi_liao_qixie_produce_license
    return collection
...
def detail_data(id):
    url = 'http://app1.sfda.gov.cn/datasearch/face3/content.jsp?' \
          'tableId=132&tableName=TABLE132&tableView=医疗器械生产企业（许可）&Id=' + str(id)
    try:
        driver.get(url)
        ...
        if '没有相关信息' in driver.page_source:
            ...
            coll.insert({
                ...
                # 生产范围
                'produce_range': trs[7].find_all('td')[1].text.strip(),
                # 发证部门
                'send_cerific_dept': trs[8].find_all('td')[1].text.strip(),
                # 发证日期
                'send_cerific_date': trs[9].find_all('td')[1].text.strip(),
                # 有效期限
                'effective_date': trs[10].find_all('td')[1].text.strip(),
                # 注
                'notes': trs[12].find_all('td')[1].text.strip(),
                # 爬取时间
